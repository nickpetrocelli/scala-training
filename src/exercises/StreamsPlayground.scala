...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  val prepended = 1 #:: s = new Cons(1, 2) (empty stream remains unevaluated)
   */
  override def #::[B >: A](element: B): MyStream[B] = new Cons(element, this)

  // issue: parameter was eagerly evaluated
  // have to call by name!!
...
object StreamsPlayground extends App {
  ...
  println(eratosthenes(naturals.tail).take(100).toList())
